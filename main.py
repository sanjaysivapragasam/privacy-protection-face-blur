...
try:  # Optional: users can comment this block if mtcnn is unavailable.
...
def select_detector(name: str) -> Callable[[np.ndarray], List[DetectionResult]]:
...
def process_single_image(
    ...
    detector = select_detector(detector_name)
...
def process_image_folder(
    ...
    for image_file in sorted(folder.iterdir()):
        if image_file.suffix.lower() not in supported_exts:
            continue
        output = process_single_image(image_file, output_dir, mode, detector_name, show)
        if output:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # Suggestions for evaluating performance and accuracy.
    if args.detector == "haar" and MTCNN is not None:
        print(
            "[TIP] Try running again with --detector mtcnn to compare detection accuracy."
        )
    ...
    print(
        ...
        "  • Log detection confidences (available with MTCNN) to compare detector reliability.\n"
        "  • Inspect qualitative differences between Gaussian blur and pixelation for privacy needs.\n"
        "  • Experiment with detection parameters (scale factor, min neighbors) to balance accuracy vs speed.\n"
    )
